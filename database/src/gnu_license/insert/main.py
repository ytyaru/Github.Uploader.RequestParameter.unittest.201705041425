...
class GnuSite(object):
                ...
                if None is not a.string:
                    ...
                    try:
                        if 'en' == self.processing_language_code:
                            self.__db.Licenses['Licenses'].insert(self.__CreateLicense(dl, dt, targetValue))
                        license = self.__db.Licenses['Licenses'].find_one(HeaderId=self.__GetHeaderId(dt))
                        if None is self.__db.Licenses['Multilingual'].find_one(LicenseId=license['Id'], LanguageCode=self.processing_language_code):
                            self.__db.Licenses['Multilingual'].insert(self.__CreateMultilingual(dt, name, self.__db.Licenses['Licenses'].find_one(HeaderId=self.__GetHeaderId(dt))['Id']))
    ...
    def __CreateLicense(self, dl, dt, targetValue):
        ...
        record = dict(
            ...
            ColorId=self.__db.Licenses['Colors'].find_one(Key=dl.get('class'))['Id'],
            Target=targetValue,
